...
export async function updateWord(id: number, changes: Partial<VocabWord>): Promise<void> {
...
export async function toggleLearned(id: number, learned: boolean): Promise<void> {
  await db.words.update(id, { learned, updatedAt: Date.now() });
}
